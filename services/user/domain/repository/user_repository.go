...
type IUserRepository interface {
	...
	// CreateUser 创建用户
	CreateUser(*model.User) (userID uint64, err error)
...
func (u *UserRepository) FindUserByID(userID uint64) (user *model.User, err error) {
	user = &model.User{}
	return user, u.mysqlDb.First(user, userID).Error
}

// CreateUser 创建用户
func (u *UserRepository) CreateUser(user *model.User) (userID uint64, err error) {
	return user.ID, u.mysqlDb.Create(user).Error
}

// DeleteUserByID 根据用户ID删除用户
func (u *UserRepository) DeleteUserByID(userID uint64) error {
	return u.mysqlDb.Where("id = ?", userID).Delete(&model.User{}).Error
}

// UpdateUser 更新用户信息
func (u *UserRepository) UpdateUser(user *model.User) error {
	return u.mysqlDb.Model(user).Update("FirstName", user.FirstName).Error
}
